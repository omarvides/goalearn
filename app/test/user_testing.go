...
// If ctx is nil then context.Background() is used.
// If service is nil then a default service is created.
func ShowUserOK(t goatest.TInterface, ctx context.Context, service *goa.Service, ctrl app.UserController, userID int) (http.ResponseWriter, *app.UserAPI) {
	// Setup service
	var (
		logBuf bytes.Buffer
		resp   interface{}
	...
	if resp != nil {
		var ok bool
		mt, ok = resp.(*app.UserAPI)
		if !ok {
			t.Fatalf("invalid response media: got %+v, expected instance of app.UserAPI", resp)
		}
		err = mt.Validate()
		if err != nil {
